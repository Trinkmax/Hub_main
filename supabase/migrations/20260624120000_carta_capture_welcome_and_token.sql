-- ────────────────────────────────────────────────────────────────────────────
-- Carta única: el formulario del club vive dentro de la carta pública y, al
-- sumarse, el cliente accede a su wallet sin loguearse.
--
-- `submit_capture` ahora:
--   1. Devuelve el `qr_token` del customer → la Server Action lo guarda en una
--      cookie para abrir la wallet (/c/[token]) desde la carta sin login.
--   2. Para customers NUEVOS, si `welcome_reward_configs.enabled`, otorga el
--      regalo de bienvenida (reward_redemption pending + welcome_reward_grants)
--      y los puntos bonus (points_transactions reason='welcome_bonus'),
--      espejando `register_customer_for_session` pero SIN sesión de mesa.
--
-- Cambia el tipo de retorno (TABLE → jsonb) por eso DROP + CREATE.
-- No hay cambios de tablas: el anidamiento de categorías ya existe.
-- ────────────────────────────────────────────────────────────────────────────
...
-- El endpoint público lo invoca con la key anon; RLS sigue siendo la defensa.
