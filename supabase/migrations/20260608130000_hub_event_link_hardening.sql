-- Hardening de los RPCs de link reserva↔evento (review correctness).
-- - El espejo se inserta SIEMPRE como invitado (customer_id = NULL): alineado con
--   el modelo "invitado sin cliente" del spec y evita chocar el índice único
--   (event_id, customer_id) cuando el mismo cliente ya es anotado directo.
-- - unlink: re-chequear 'cancelled' tras el `for update` (paridad con el motor).
...
begin
  ...
  if v_existing.id is null then
    -- Anotado "invitado": customer_id NULL (la ficha CRM, si existe, vive en la reserva).
...
begin
  ...
  -- Carrera: si quedó cancelado entre el read sin lock y el lock, salir limpio.
