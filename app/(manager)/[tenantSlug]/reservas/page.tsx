...
import { computeDaySegments } from '@/lib/salon/segments'
...
// La lista de reservas: día (default), semana, mes o rango, con buscador,
// filtros, «Pasar lista», exportar y «Nueva reserva». Volvió al menú el
// 22/09/2026, un día después de haberse ocultado detrás del calendario: la
// anfitriona trabaja con la lista. Se reserva desde los dos lados y al guardar
// se vuelve a la pantalla de origen (ver lib/salon/calendar-links.ts).
...
export default async function ReservasPage({
  ...
  const zone =
    typeof sp.zone === 'string' && salonZoneEnum.safeParse(sp.zone).success
      ...
      : undefined
  // Un uuid y nada más: el id va adentro de un `or()` de PostgREST.
  const managerId =
    typeof sp.manager === 'string' && z.uuid().safeParse(sp.manager).success
      ? sp.manager
      : undefined
  ...
  // link viejo) se ignora: el día cae en hoy en vez de tirar la página.
  const isoDayOrUndefined = (value: unknown) =>
    typeof value === 'string' && isoDaySchema.safeParse(value).success ? value : undefined
  const fromParam = isoDayOrUndefined(sp.from)
  const toParam = isoDayOrUndefined(sp.to)
  const rangeMode = Boolean(fromParam || toParam)
  const today = todayInCordoba()
  const day = rangeMode ? undefined : (isoDayOrUndefined(sp.day) ?? today)
  ...
  // el contador por servicio y la reserva "nueva" no necesitan el listado,
  ...
  let loaded: [
    ...
    Awaited<ReturnType<typeof getDaySegmentsSnapshot>> | null,
  ...
  try {
    loaded = await Promise.all([
      ...
      // Contador del día POR SERVICIO: el día entero (sin los filtros de la
      // pantalla), sus eventos y los cupos resueltos, en una sola lectura. Es
      // lo mismo que usa el calendario, así la cena dice lo mismo en los dos
      // lados. Sus eventos alimentan también el renglón de hitos. En modo
      // rango, el volumen del período (un cupo no significa nada sumando días).
      day ? getDaySegmentsSnapshot({ tenantId: access.tenant.id, date: day }) : null,
      !day && (dateFrom || dateTo)
        ? getRangeReservationTotals({
            tenantId: access.tenant.id,
      ...
      day
        ...
        : null,
      // Los festejos del día, con su propia query: los hitos hablan del DÍA, no de la
  ...
  // El día cortado por servicio con la cuenta única (lib/salon/segments.ts):
  // cada servicio contra su cupo, nunca el total del día contra PA + PB.
  const daySegments =
    day && daySnapshot
      ? computeDaySegments({
          date: day,
          reservations: daySnapshot.reservations,
          events: daySnapshot.events,
          caps: daySnapshot.caps,
        })
      : null
  ...
  const dayHighlights =
    day && daySnapshot && daySegments
      ? buildDayHighlights({
          events: daySnapshot.events,
          reservations: dayCelebrations ?? [],
          usedByEvent: eventUsedById(daySegments),
        })
      : []
  ...
  // paginar. Con los valores YA validados de arriba, no los crudos de `sp`:
  // una fecha rota que la lista descartó no puede cambiar el modo del export.
  const exportHref = reservasExportHref(tenantSlug, {
    day,
    from: fromParam,
    to: toParam,
    q,
      ...
      <div className="space-y-3" data-tour="reservas-dia">
        ...
        {rangeMode ? (
          ...
          <DayNavigator
            ...
            day={day}
            today={today}
            segments={daySegments ? activeDaySegments(daySegments) : []}
            dayLabel={counterDayLabel(day)}
          />
        ) : null}
